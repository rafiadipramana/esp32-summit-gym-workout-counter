...
// #define START_PAUSE_BUTTON_PIN 32
// #define RESET_BUTTON_PIN 35
// #define SAVE_BUTTON_PIN 34
...
//   static int lastStartPauseButtonState = HIGH;
//   static int lastResetButtonState = HIGH;
//   static int lastSaveButtonState = HIGH;

//   int currentStartPauseButtonState = digitalRead(START_PAUSE_BUTTON_PIN);
...
//   static unsigned long pauseStartTime = 0; // Track the start time of pause
//   static unsigned long pausedTime = 0;    // Accumulate paused time

//   if (currentStartPauseButtonState == LOW && lastStartPauseButtonState == HIGH)
//   {
//     if (isCounting && !isPaused)
//     {
//       isPaused = true; // Pause counting
//       pauseStartTime = currentMillis; // Record the time when paused
//       Serial.println("Counting paused");
//     }
//     else if (isCounting && isPaused)
//     {
//       isPaused = false; // Resume counting
//       pausedTime += (currentMillis - pauseStartTime); // Accumulate the paused time
...
//   if (isCounting && !isPaused)
//   {
//     unsigned long elapsedTime = currentMillis - startTime - pausedTime; // Calculate the stopwatch time
...
//       writer.create(timeInMillisJson, "timeInMillis", elapsedTime);
